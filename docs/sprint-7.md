...
A working tree can contain files that a fresh CI checkout will not, and `ccc plan` locally must resolve exactly what CI will publish. When the content root lies inside a Git work tree, every selected source must therefore be tracked by Git: each file resolved from a directory entry, each directly listed file, and the Markdown source of a generated notebook (or the notebook itself when it is the authoritative source with no Markdown). The one deliberate exemption is a private paired notebook alongside tracked Markdown, which may remain untracked or ignored, since the Markdown is authoritative there. An untracked selected source is an error naming the file and the entry that selected it. When the content root is not inside a Git work tree, the check is skipped and the plan records that it was skipped. Tracked status is used only to reject an ambiguous selection; it never adds a file to the export.
...
- With the content root in a Git work tree, an untracked expanded member, directly listed file, or generated-notebook Markdown source is an error naming the file and entry, while an untracked private notebook beside tracked Markdown is permitted; outside a work tree the check is skipped and the plan records the skip.
